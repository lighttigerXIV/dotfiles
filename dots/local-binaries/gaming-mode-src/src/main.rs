...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if gaming_mode {
        ...
        Command::new("kscreen-doctor")
            .arg(format!("output.{screen_id}.mode.{work_quality_id}"))
            .spawn()?;

        Command::new("notify-send")
            .arg("🧳 Gaming Mode Disabled")
            .spawn()?;
        ...
        Command::new("qdbus6")
            .args(["qdbus6", "org.kde.KWin", "/KWin", "reconfigure"])
            .spawn()?;
    ...
    Command::new("kscreen-doctor")
        .arg(format!("output.{screen_id}.mode.{gaming_quality_id}"))
        .spawn()?;
